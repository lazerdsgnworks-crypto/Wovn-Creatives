...
const getInitialPage = (): AppPage => {
  ...
  if (path === "/chat" || hash === "#chat" || hash === "#/chat" || search.get("page") === "chat") return "chat";
...
export default function App() {
  ...
  return (
    <div className="relative min-h-screen w-full bg-background text-foreground font-sans tracking-[-0.02em] selection:bg-zinc-300 dark:selection:bg-zinc-700 selection:text-zinc-950 dark:selection:text-white transition-colors duration-200">
      {/* Top Floating & Fixed Navigation Bar */}
      {currentPage !== "chat" && (
        <Header
          currentPage={
      ...
      {currentPage === "chat" && (
        <ChatPage
          ...
          onNavigate={(p) => navigateTo(p as AppPage)}
        />
      )}
      ...
      {currentPage === "home" && (
        <main>
          ...
          <section
            id="hero-top"
            className="relative h-[58vh] sm:h-[72vh] md:h-screen w-full flex flex-col justify-between overflow-hidden bg-background pt-14 sm:pt-16"
          >
            <ImageStreamHero
              images={IMAGES}
              cards={8}
            ...
            >
              {/* Hero text: balanced spacing below the header */}
              <div className="pointer-events-none relative z-20 flex w-full flex-col items-center pt-8 sm:pt-10 md:pt-12 text-center">
      ...
      {/* Studio Footer */}
      {currentPage !== "our-work" && currentPage !== "chat" && <Footer16 onNavigate={navigateTo} />}
