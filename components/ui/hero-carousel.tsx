...
export interface HeroCarouselItem {
  ...
  image: string
  /** Byline printed beside the headline, e.g. "WOVN CREATIVES". @default undefined */
  credit?: string
  /** Target link when the credit/name is clicked. @default "https://www.instagram.com/wovncreatives" */
  creditUrl?: string
      ...
      >
        <div className="flex w-full flex-wrap items-end gap-x-[6vw] gap-y-2">
          <AnimatePresence mode="popLayout" initial={false}>
            <motion.h2
              key={index}
              className="font-normal leading-[0.88] tracking-[-0.03em]"
          ...
          {active.credit ? (
            <motion.a
              href={active.creditUrl || "https://www.instagram.com/wovncreatives"}
              target="_blank"
              rel="noopener noreferrer"
              key={`credit-${index}`}
              className="group inline-flex items-center gap-1 font-mono uppercase tracking-[0.14em] text-white/80 hover:text-white transition-colors duration-200 cursor-pointer select-none"
              style={{ fontSize: label }}
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.8 }}
              transition={{ duration: 0.5, delay: 0.1 }}
              title="Visit @wovncreatives on Instagram"
              id="work-item-instagram-link"
            >
              <span>{active.credit}</span>
              <span className="inline-block text-[1.15em] leading-none transition-transform duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">
                ↗
              </span>
            </motion.a>
          ) : null}
